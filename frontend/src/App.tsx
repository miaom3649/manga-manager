...
function migratedStorage(key:string,legacyKey:string):string|null{const current=localStorage.getItem(key);if(current!==null)return current;const legacy=localStorage.getItem(legacyKey);if(legacy!==null)localStorage.setItem(key,legacy);return legacy}
const tokenKey = `hmanga-token:${location.host}`;
const legacyTokenKey = `hlibrary-token:${location.host}`;
const computersKey = "hmanga-computer-entries";
const isAuthor = (tag:WorkTag) => tag.groupName === "作者";
const tagClass = (tag:WorkTag) => isAuthor(tag) ? "author-tag" : tag.groupId == null ? "ungrouped" : "";

function savedComputers():string[]{try{return JSON.parse(migratedStorage(computersKey,"hlibrary-computer-entries")??"[]") as string[]}catch{return[]}}
...
function Modal({children,close,wide=false,warning=false}:{children:React.ReactNode;close?:()=>void;wide?:boolean;warning?:boolean}){
...
<section className={`panel floating-card ${wide?"wide":""} ${warning?"warning-card":""}`}>{children}</section>
</div>
}
...
function Pair({onPaired}:{onPaired:(token:string)=>void}){
  ...
  async function submit(event:FormEvent){event.preventDefault();setError("");try{const result=await api<{token:string}>("/api/pair","",{method:"POST",body:JSON.stringify({code,name}),headers:{Authorization:""}});localStorage.setItem(tokenKey,result.token);onPaired(result.token);rememberComputer(location.origin);history.replaceState(null,"",location.pathname)}catch(reason){setError(reason instanceof Error?reason.message:t("error.pairing_failed"))}}
...
</div>}<p className="center-text muted">{count?tf("cover.page_summary",{page:index+1,pages:count,member:pages.data?.items[index]??""}):t("label.no_available_images")}</p>
...
<AuthImage path={`/api/works/${id}/thumbnail`} token={token}/>
...
<input placeholder={t("label.search_tags_or_hidden_groups")} value={tagSearch} onChange={e=>setTagSearch(e.target.value)}/>
...
function FilterPanel({token,value,change,close}:{token:string;value:Filters;change:(value:Filters)=>void;close:()=>void}){
  ...
  const revealAuthors=search.trim().includes("作者");const visibleTags=tags.data?.filter(tag=>!isAuthor(tag)||revealAuthors||value.tagIds.includes(tag.id));
...
<p>{tf("tag.mobile_delete_confirm",{name:deleteTag.name,count:deleteTag.works??0})}</p>
<div className="split-actions">
...
function EditTag({tag,groups,token,done}:{tag:Tag;groups:Group[];token:string;done:()=>void}){const[name,setName]=useState(tag.rawName??tag.name);const[groupId,setGroupId]=useState<number>(tag.groupId??groups.find(group=>group.name==="类别")?.id??0);return <>
<h2>{t("action.edit_tag")}</h2>
<label>{t("label.name")}<input maxLength={groups.find(group=>group.id===groupId)?.name==="作者"?200:5} value={name} onChange={e=>setName(e.target.value)}/>
</label>
...
function SettingsPanel({token,close,disconnect}:{token:string;close:()=>void;disconnect:()=>void}){const[theme,setTheme]=useState(()=>migratedStorage("hmanga-theme","hlibrary-theme")??"system");const[computerUrl,setComputerUrl]=useState("");const device=useQuery({queryKey:["current-device"],queryFn:()=>api<{name:string}>("/api/devices/me",token)});const computer=useQuery({queryKey:["computer"],queryFn:()=>api<{computerName:string}>("/api/library/status",token)});function change(value:string){setTheme(value);localStorage.setItem("hmanga-theme",value);document.documentElement.dataset.theme=value}return <Modal close={close}>
...
<div className="cover">
<AuthImage path={`/api/works/${work.id}/thumbnail`} token={token}/>
</div>
<div className="meta">
<h2>{work.title}</h2>
<p className="work-number">{work.kind==="comic"?work.number||"":work.fileName}</p>
...
</div>}{showFilters&&<FilterPanel token={token} value={filters} change={value=>{setFilters(value);setPage(1)}} close={()=>setShowFilters(false)}/>} {showNotifications&&<NotificationPanel token={token} close={()=>setShowNotifications(false)}/>} {showSettings&&<SettingsPanel token={token} close={()=>setShowSettings(false)} disconnect={disconnect}/>}</main>}

export default function App(){const[token,setToken]=useState(()=>migratedStorage(tokenKey,legacyTokenKey)??"");useEffect(()=>{document.documentElement.dataset.theme=migratedStorage("hmanga-theme","hlibrary-theme")??"system"},[]);const disconnect=useCallback(()=>{if(token)void fetch("/api/devices/me",{method:"DELETE",headers:{Authorization:`Bearer ${token}`}});localStorage.removeItem(tokenKey);sessionStorage.removeItem(libraryViewKey);setToken("")},[token]);return token?<Library token={token} disconnect={disconnect}/>:<Pair onPaired={setToken}/>}
